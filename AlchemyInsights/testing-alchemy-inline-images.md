---
title: Alchemy 인라인 이미지 테스트
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: ea91a0ef0e003d1a2444d4d5e72ccbdd608bf102
ms.sourcegitcommit: 1d98db8acb9959aba3b5e308a567ade6b62da56c
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/22/2019
ms.locfileid: "36507169"
---
# <a name="access-the-domains-page"></a><span data-ttu-id="500f4-102">도메인 액세스 페이지</span><span class="sxs-lookup"><span data-stu-id="500f4-102">Access the Domains Page</span></span>

<span data-ttu-id="500f4-103">도메인 페이지에 액세스 하려면 아래에서이 아이콘을 클릭 하면 됩니다.</span><span class="sxs-lookup"><span data-stu-id="500f4-103">To access the domains page, just click this icon below:</span></span>

  
<img src='data:image/png;base64,'/>

<span data-ttu-id="500f4-104">그런 다음 도메인을 추가, 수정 또는 제거할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="500f4-104">Then you can add, modify, or remove your domain.</span></span>
<span data-ttu-id="500f4-105">자세한 내용은 도메인 FAQ를 참조 [하세요.](https://support.office.com/article/1272bad0-4bd4-4796-8005-67d6fb3afc5a.aspx)</span><span class="sxs-lookup"><span data-stu-id="500f4-105">For more information, see the [Domains FAQ.](https://support.office.com/article/1272bad0-4bd4-4796-8005-67d6fb3afc5a.aspx)</span></span>
  